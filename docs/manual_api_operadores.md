...
### 4.1 Iniciar Ruta (`POST /api/iniciar_ruta.php`)
...
- **Body Params**:
  ...
  - `km_inicial` (float, obligatorio, > 0)
  - `foto_km_inicial` (archivo o string base64, obligatorio)
...
curl -X POST "https://spbservicios.com/spb/api/iniciar_ruta.php" \
     ...
     -F "id_ruta=236" \
     -F "km_inicial=45200.5" \
     -F "foto_km_inicial=@odometro_inicio.jpg"
```
